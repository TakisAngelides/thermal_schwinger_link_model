...
def write_dag():
    ...
    max_steps_list = [50, 500]
    delta_beta_list = [0.01, 0.001]
    save_steps_lists = [[10, 20, 30, 40, 50], [100, 200, 300, 400, 500]]
    for S in [1, 2, 3, 4]:
        for x in [10]:
            for mg in [0.5]:
                for N in [40]:
    ...
    for file in os.listdir(path_to_HDF5):
        ...
        try:
            ...
            for beta_idx, beta in enumerate(beta_list):
                ...
                x, y = zip(*[(xi, yi) for xi, yi in zip(x, y) if yi >= 1e-13])
