            ...
            for (mg_key, mg_value) in data
                for (S_key, S_value) in mg_value
                    for (beta_key, beta_value) in S_value
                        ...
                        # Create bar plot
                        p = plot(x_plot, y_plot; st=:bar, label="", xlabel="String Length", ylabel="Normalized Frequency")

                        # Annotate each bar with its value and adjust the font size
                        for (x, y) in zip(x_plot, y_plot)
                            annotate!(x, y, text(string(round(y, digits=6)), :center, 8, rotation=90))  # 8 is the font size, adjust as needed
                        end
